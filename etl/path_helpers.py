...
def get_inbox_path(config_object_id: ObjectId,
    ...
    """
    return (_compute_config_path(config_object_id, cfg.inbox_dir,
                                 filename(file_object_id))
            if file_object_id is not None else _compute_config_path(
                config_object_id, cfg.inbox_dir))


def get_processing_path(config_object_id: ObjectId,
                        cfg: FileProcessorConfig,
                        file_object_id: ObjectId = None) -> ObjectId:
    ...
    :param file_object_id: An optional object to locate in the directory
    """
    return (_compute_config_path(config_object_id, cfg.processing_dir,
                                 filename(file_object_id))
            if file_object_id is not None else _compute_config_path(
                config_object_id, cfg.processing_dir))
...
def get_archive_path(config_object_id: ObjectId,
    ...
    """
    if cfg.archive_dir:
        return (_compute_config_path(config_object_id, cfg.archive_dir,
                                     filename(file_object_id))
                if file_object_id is not None else _compute_config_path(
                    config_object_id, cfg.archive_dir))
    return None


def get_error_path(config_object_id: ObjectId,
                   cfg: FileProcessorConfig,
                   file_object_id: ObjectId = None) -> ObjectId:
    ...
    :param file_object_id: An optional object to locate in the directory
    """
    return (_compute_config_path(config_object_id, cfg.error_dir,
                                 filename(file_object_id))
            if file_object_id is not None else _compute_config_path(
                config_object_id, cfg.error_dir))
...
def parent(object_id: ObjectId) -> ObjectId:
    ...
    return ObjectId(object_id.namespace,
                    str(PurePosixPath(object_id.path).parent))


def glob_matches(object_id: ObjectId, config_object_id: ObjectId,
                 cfg: FileProcessorConfig) -> bool:
    ...
    try:
        file_path = str(
            PurePosixPath(object_id.path).relative_to(
                parent(config_object_id).path))
...
def mimetype_matches(object_id: ObjectId, config_object_id: ObjectId,
                     cfg: FileProcessorConfig, object_store: ObjectStore,
                     rest_client: RestClient, tika_host: str) -> bool:
    ...
    try:
        ...
        mimetypes = cfg.handled_mimetypes.split(
            ",") if cfg.handled_mimetypes else []

        try:
            response = rest_client.make_request(f'{tika_host}/detect/stream',
                                                method='put',
                                                data=file_object,
                                                headers={
                                                    'Content-Type':
            ...
            if not response or response.status_code != HTTPStatus.OK:
                LOGGER.warning(
                    f'Unexpected response from Tika service: {response.content}'
                )
            else:
...
def processor_matches(object_id: ObjectId, config_object_id: ObjectId,
                      cfg: FileProcessorConfig, object_store: ObjectStore,
                      rest_client: RestClient, tika_host: str,
                      enable_tika: bool) -> bool:
    matches = False

    if enable_tika:
        matches = mimetype_matches(object_id, config_object_id, cfg,
                                   object_store, rest_client, tika_host)
