...
class GeneralEventProcessor:
    ...
    async def checkForProcessingRecords(self):
        missed_files: Optional[List[FileObject]] = await self._database.query(
        ...
        if missed_files:
            ...
            for stalled_file in missed_files:
                ...
                src_object_id = ObjectId(stalled_file.bucket_name,
                                         stalled_file.file_name)
                dest_object_id = ObjectId(stalled_file.bucket_name,
                                          f'{new_path}')
                ...
                self.logger.error(
                    f"Moving file: {stalled_file.file_name} back to original filename: {new_path} to restart processing..."
                )

                try:
                    self._object_store.move_object(src_object_id,
                                                   dest_object_id, metadata)
                    await self._database.delete_file(rowid=stalled_file.id)
                except Exception as e:
                    self.logger.error(
                        f"Error restoring file.  Possible database/Minio mismatch: {e}"
                    )
    ...
    async def process(self, evt_data: Dict) -> None:
        ...
        if any([evt.object_id.path.endswith(i)
                for i in file_suffix_to_ignore]):
            pass
        else:
            ...
            if evt.event_type == EventType.Delete:
                ...
                if evt.original_filename:
                    ...
                    self._object_store.move_object(evt.object_id,
                                                   dest_object_id, metadata)
    ...
    async def _file_put(self, object_id: ObjectId, uuid: str) -> bool:
        ...
        processor_dict: Dict[
            ObjectId,
            FileProcessorConfig] = await self._toml_processor.get_processors.remote(
            )
        for config_object_id, processor in processor_dict.items():
            if (parent(object_id) != get_inbox_path(
                    config_object_id, processor) or not processor_matches(
                        object_id, config_object_id, processor,
                        self._object_store, self._rest_client,
                        settings.tika_host, settings.enable_tika)):
                # File isn't in our inbox directory or filename doesn't match our glob pattern
                continue

            self.logger.info(
                f'matching ETL processing config found, processing {config_object_id}'
            )

            # Hypothetical file paths for each directory
            processing_file = get_processing_path(config_object_id, processor,
                                                  object_id)
            ...
            error_log_file = get_error_path(
                ...
                rename(object_id, error_log_file_name))

            job_id = short_uuid()
            self._message_producer.job_created(job_id, filename(object_id),
                                               filename(config_object_id),
                                               'castiron')
            ...
            with tempfile.TemporaryDirectory() as work_dir:
                ...
                self._object_store.download_object(processing_file,
                                                   str(local_data_file))
                ...
                with open(base_path / 'out.txt', 'w') as out, \
                    ...
                    if processor.python.supports_pizza_tracker:
                        ...
                        self.logger.warning(
                            f'processor {config_object_id} does not support pizza tracker'
                        )
                    ...
                    if success:
                        ...
                        if archive_file:
                            ...
                            self._object_store.move_object(
                                processing_file, archive_file, metadata)

                            await self._database.update_status_and_fileName(
                                uuid, STATUS_SUCCESS, archive_file.path)
                            self.logger.info(
                                f'file processing success, moving to archive {archive_file.path}'
                            )

                        self._message_producer.job_evt_status(
                            job_id, 'success')
                    else:
                        # Failure. mv to failed
                        metadata['status'] = STATUS_FAILED
                        self._object_store.move_object(processing_file,
                                                       error_file, metadata)

                        self.logger.warn(
                            f'file processing failed, moving to error location {error_file.pat}'
                        )
                        await self._database.update_status_and_fileName(
