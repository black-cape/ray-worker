...
class MinioObjectStore(ObjectStore):
    ...
    def upload_object(
        self, dest: ObjectId, src_file: str, metadata: Optional[Dict]
    ) -> None:
        self._minio_client.fput_object(
            bucket_name=dest.namespace,
    ...
    def move_object(
        self, src: ObjectId, dest: ObjectId, metadata: Optional[Dict] = None
    ) -> None:
        if metadata:
            ...
            self._minio_client.copy_object(
                dest.namespace, dest.path, CopySource(src.namespace, src.path)
            )
    ...
    def ensure_directory_exists(self, directory: ObjectId) -> None:
        if not next(
            self._minio_client.list_objects(directory.namespace, directory.path), None
        ):
    ...
    def parse_notification(self, evt_data: Any) -> ObjectEvent:
        ...
        event_type = (
            EventType.Delete
            if evt_data["EventName"].startswith("s3:ObjectRemoved")
            else EventType.Put
        )
    ...
    def list_objects(
        self, namespace: str, path: Optional[str] = None, recursive=False
    ) -> Iterable[ObjectId]:
