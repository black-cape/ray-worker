...
class PizzaTracker:
    ...
    def __init__(
        self, message_producer: MessageProducer, working_dir: str, job_id: str
    ):
    ...
    def process(self):
        ...
        while True:
            try:
                ...
                handler = getattr(f"_handle_{cmd.lower()}")
                handler(args)
    ...
    def _handle_progress(self, data: str):
        ...
        if progress is not None and 0.0 <= progress <= 1.0:
            logger.debug(
                "pizza tracker is logging progress to kafka. job_id: %s, progress: %s",
                self.job_id,
                progress,
            )
    ...
    def __enter__(self):
        ...
        self.f_pipe = TextIOWrapper(
            BufferedReader(
                FileIO(
                    os.open(self.pipe_file_name, os.O_RDONLY | os.O_NONBLOCK), mode="r"
                )
            )
        )
        return self
